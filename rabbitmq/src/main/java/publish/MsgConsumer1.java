...
public class MsgConsumer1 {
    public static void main(String[] args) throws IOException, TimeoutException {
        ...
        // 5.其他额外的参数
        channel.queueDeclare(RabbitmqConstant.QUEUE_PAY_1,false,false,false,null);
        // 1.队列名称
        // 2.交换机名称
        // 3.routeKey 在发布订阅模式暂时用不到 在routekey模式和topic模式用到
        channel.queueBind(RabbitmqConstant.QUEUE_PAY_1,RabbitmqConstant.EXCHANGE_PAY_SUCCESS,"");
        // 参数1代表消费完一条消息之后才进行确认 之后再去队列获取消息
        channel.basicQos(1);
        // 1.队列名称
        // 2.是否自动确认消息 false 手动代码确认消息,rabbitmq推荐
        // 3.消费者对象
        channel.basicConsume(RabbitmqConstant.QUEUE_PAY_1,false,new DefaultConsumer(channel){
            @Override
